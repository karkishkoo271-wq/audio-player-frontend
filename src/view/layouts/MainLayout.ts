...
export class MainLayout {
  ...
  private sortField: 'title' | 'album' | null = null;
  ...
  /**
   * ✅ СОРТИРОВКА ТРЕКОВ ПО ПОЛЮ
   */
  private sortTracks(field: 'title' | 'album'): void {
  ...
  private async loadFavorites(): Promise<void> {
    try {
      ...
      this.favorites = new Set(favTracks.map((t: Track) => t.id));
  ...
  private renderTracks(): void {
    ...
    // ✅ КЛИКАБЕЛЬНЫЕ ЗАГОЛОВКИ С СОРТИРОВКОЙ
    const headers = el('div.track-list__headers',
      el('div.track-header__number', '№'),
      el('div.track-header__cover'),
      // НАЗВАНИЕ — кликабельно
      el('div.track-header__title', {
        textContent: 'НАЗВАНИЕ',
        style: 'cursor: pointer; user-select: none; display: flex; align-items: center; gap: 4px;',
        title: 'Сортировать по названию',
        onclick: () => this.sortTracks('title')
      },
        ' ▲',
        el('span', {
          style: `color: ${this.sortField === 'title' ? 'var(--color-primary)' : 'transparent'}`,
          textContent: this.sortDirection === 'asc' ? '↑' : '↓'
        })
      ),
      // АЛЬБОМ — кликабельно
      el('div.track-header__album', {
        textContent: 'АЛЬБОМ',
        style: 'cursor: pointer; user-select: none; display: flex; align-items: center; gap: 4px;',
        title: 'Сортировать по альбому',
        onclick: () => this.sortTracks('album')
      },
        ' ▲',
        el('span', {
          style: `color: ${this.sortField === 'album' ? 'var(--color-primary)' : 'transparent'}`,
          textContent: this.sortDirection === 'asc' ? '↑' : '↓'
        })
      ),
    ...
    const cardElements: HTMLElement[] = this.tracks.map((track: Track, index: number) => {
      ...
      const card = new TrackCard({
        ...
        onToggleFavorite: (trackId: string) => this.handleFavoriteToggle(trackId)
      });
  ...
  private handleSearch(query: string): void {
    ...
    const filtered = this.tracks.filter((track: Track) =>
    ...
    if (filtered.length === 0) {
      ...
      const cardElements: HTMLElement[] = filtered.map((track: Track, index: number) => {
        const card = new TrackCard({
          track,
          number: index + 1,
          onPlay: (t: Track) => {
  ...
  private async loadFavoritesOnly(): Promise<void> {
    ...
    try {
      ...
      this.favorites = new Set(favorites.map((t: Track) => t.id));
  ...
  private async loadMoreTracks(): Promise<void> {
    try {
      ...
      const newCards = tracks.map((track: Track, index: number) => {
